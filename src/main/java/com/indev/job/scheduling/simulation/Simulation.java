...
public class Simulation {
    ...
    public RunResult simulate(int runCount) {
        ...
        return new RunResult(globalAverageWaitingTime, globalAverageTurnAroundTime);
    }
    ...
    private RunResult singleRun() {
        ...
        for (int i = 0; i < jobsCount; i++) {
            Job job = generateRow("P" + i);
            schedulerAlgorithm.add(job);
        }
        schedulerAlgorithm.process();
        schedulerResultPrinter.printSchedulerResult(schedulerAlgorithm);
        return new RunResult(schedulerAlgorithm.getAverageWaitingTime(), schedulerAlgorithm.getAverageTurnAroundTime());
    }

    private Job generateRow(String processId) {
